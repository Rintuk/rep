...
async def dashboard(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    total_locked_gross = 256.0 + 1348.74 + sum(f.locked_forex_pnl / get_investor_share(f) for f in all_fins_forex if getattr(f, "locked_forex_pnl", 0.0) != 0.0 and get_investor_share(f) > 0)
