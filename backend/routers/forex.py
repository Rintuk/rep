...
async def admin_forex_overview(db: AsyncSession = Depends(get_db)):
    ...
    if snap:
        ...
        for t in all_trade_rows:
            ...
            if len(trades) >= 30 and trade_date < today:
                break
    ...
    for u in investors:
        ...
        if inv > 0 and snap and net_invested_pool > 0:
            ...
            # Admin only gets fee from profits
    ...
    total_locked_gross = 299.0
    for u in investors:
        fin = fins_map.get(u.id)
        if fin and fin.locked_forex_pnl != 0.0:
            inv_share = get_investor_share(fin)
            if inv_share > 0:
                total_locked_gross += fin.locked_forex_pnl / inv_share
