...
async def approve_withdrawal(request_id: str, actual_amount: float, db: AsyncSession = Depends(get_db)):
    ...
    if snap:
        ...
        total_inv = (await db.execute(select(func.sum(UserFinancials.investment_usdt)))).scalar() or 0.0
        total_wd = (await db.execute(select(func.sum(UserFinancials.withdrawal_usdt)))).scalar() or 0.0
        ref = start + total_inv - total_wd - actual_amount
        if ref <= 0:
            ref = snap.net_invested if snap.net_invested > 0 else start
        new_pnl_pct = round((pool_total_new - ref) / ref * 100, 4) if ref > 0 else 0.0

    await _migrate_pnl_internal(db, override_crypto_pct=current_pnl_pct, final_crypto_pct=new_pnl_pct)
...
@router.post("/admin/reset-ref-bonus-true")
async def admin_reset_ref_bonus_true(db: AsyncSession = Depends(get_db)):
    """Reset referral bonuses to 0 by offsetting locked_ref_bonus"""
    from routers.dashboard import _calc_referral_tree
    ...
    for u in all_users:
        ...
        # We want the new crypto_ref and forex_ref to be 0
        # So we subtract the current total from the locked fields
