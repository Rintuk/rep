...
export async function approveForexDepositFromPool(id: string, actual_amount: number) {
  const res = await api.post(`/auth/admin/deposits/${id}/forex-approve-from-pool`, null, { params: { actual_amount } });
  return res.data;
}
...
export async function externalDeposit(user_id: string, amount: number) {
  const res = await api.post("/auth/admin/external-deposit", { user_id, amount });
  return res.data;
}

export async function forexExternalDeposit(user_id: string, amount: number) {
  const res = await api.post("/auth/admin/forex-external-deposit", { user_id, amount });
  return res.data;
}
...
export async function resetForexNotebook() { const res = await api.post('/auth/admin/notebook/reset-forex'); return res.data; }
